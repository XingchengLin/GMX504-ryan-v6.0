...
#ifdef SBM_RUMB
        case F_GAUSS:
            newparam->gauss.amp   = old[0];
            newparam->gauss.mu    = old[1];
            newparam->gauss.sigma = old[2];
            newparam->gauss.rad   = old[3];
            break;
        case F_BAREGAUSS:
            newparam->baregauss.amp   = old[0];
            newparam->baregauss.mu    = old[1];
            newparam->baregauss.sigma = old[2];
            break;
        case F_TWOGAUSS:
            newparam->twogauss.amp    = old[0];
            newparam->twogauss.mu1    = old[1];
            newparam->twogauss.sigma1 = old[2];
            newparam->twogauss.mu2    = old[3];
            newparam->twogauss.sigma2 = old[4];
            newparam->twogauss.rad    = old[5];
            break;
        case F_PUMBRELLA:
            newparam->pumbrella.amp    = old[0];
            newparam->pumbrella.mu     = old[1];
            newparam->pumbrella.sigma  = old[2];
            break;
#endif // SBM_RUMB
