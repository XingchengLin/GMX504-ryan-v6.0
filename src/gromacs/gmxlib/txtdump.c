...
void pr_iparams(FILE *fp, t_functype ftype, t_iparams *iparams)
{
	...
	case F_VDW14:
        ...
        case  F_CBTDIHS:
            ...
            fprintf(fp, "\n");
            break;
	 /* These are dummy states for Gaussian types, just to make other Gromacs standard types being able to be
	  * dumped out, without these gmxdump will complain no gauss type existed;
	  * In the future we can code something to make Gaussian type being dumped out explicitly;
	  */
	case  F_BAREGAUSS:
	    fprintf(fp, "\n");
	    break;
